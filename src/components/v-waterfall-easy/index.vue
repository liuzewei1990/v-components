    ...
    <div class="vue-waterfall-easy-scroll" ref="scrollEl">
      ...
      <div class="vue-waterfall-easy" :style="isMobile? '' :{width: colWidth*cols+'px',left:'50%', marginLeft: -1*colWidth*cols/2 +'px'}">
        <div class="img-box" v-for="(v,i) in imgsArr_c" :key="i" :class="[cardAnimationClass, {__err__: v._error}]" :style="{padding: (isMobile ? mobileGap : gap)/2+'px', width: isMobile ? '' : colWidth+'px'}">
          <component class="img-inner-box" :is="isRouterLink  && linkRange=='card' ? 'router-link' : 'alink'" :data-index="i" :to="linkRange=='card' ? v[hrefKey] : false">
            <component class="img-wraper" v-if="v[srcKey]" :is="isRouterLink && linkRange=='img' ? 'router-link' :'alink'" :to="linkRange=='img' ? v[hrefKey] : false " :style="{width:imgWidth_c + 'px',height:v._height ? v._height+'px':false}">
